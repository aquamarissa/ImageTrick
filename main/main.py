...
import subprocess
# import for 3d rotation on image
...
class MyWin(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.ui.actionMake_collage.triggered.connect(lambda: self.rotate_along_axis())
        self.ui.addButton.clicked.connect(lambda: self.photoDataInArray())
    ...
    def photoDataInArray(self):
        self.photo.append(self.task4[0])
        self.longitude.append(int(self.ui.longitudeEdit.text()))
        self.latitude.append(int(self.ui.latitudeEdit.text()))
        self.altitude.append(int(self.ui.altitudeEdit.text()))
        self.roll.append(int(self.ui.rollEdit.text()))
        self.pitch.append(int(self.ui.pitchEdit.text()))
        self.yaw.append(int(self.ui.yawEdit.text()))

    def rotate_along_axis(self):
        print(self.parties_h)
        way = Way(self.photo, self.longitude, self.latitude, self.altitude, self.roll, self.pitch, self.yaw,
                  self.parties_h, self.parties_w)
    ...
    def setAllOptions(self):
        try:
            ...
            if int(self.ui.lineEdit_9.text()) < 0:
                ...
                self.parties_w = self.image_window.parties_h
                self.parties_h = self.image_window.parties_w
