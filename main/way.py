...
class Way:
    def __init__(self, args, longitude, latitude, altitude, roll, pitch, yaw, height, width):
        ...
        self.textures = []
        self.polygons = []
        self.meters_per_pixel_h = width / 600
        self.meters_per_pixel_w = height / 175
        for i in range(len(self.photos)):
            self.createPoly(math.sqrt(0.8**2 + 1.25**2), self.roll[i], self.latitude[i] * self.meters_per_pixel_h,
                            self.longitude[i] * self.meters_per_pixel_w, 1.25)
        self.display()
